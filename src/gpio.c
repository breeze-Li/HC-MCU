...
//添加
en_result_t My_Gpio_SetFunc(uint8_t u8Port, uint8_t u8Pin, uint32_t	FuncSel)
{
		...
		switch(u8Port)
		{
				case	0:
				{
						*((volatile uint32_t *)(&M0P_GPIO->P01_SEL_f	+	u8Pin	-	1))	=	FuncSel;
						break;
				}
				case	1:
				{
						*((volatile uint32_t *)(&M0P_GPIO->P14_SEL_f	+	u8Pin	-	4))	=	FuncSel;
						break;
				}
				case	2:
				{
						*((volatile uint32_t *)(&M0P_GPIO->P23_SEL_f	+	u8Pin	-	3))	=	FuncSel;
						break;
				}
				case	3:
				{
						*((volatile uint32_t *)(&M0P_GPIO->P31_SEL_f	+	u8Pin	-	1))	=	FuncSel;
						break;
				}
		}
