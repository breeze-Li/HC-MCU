 ...
 * \param   [in]  enAdtUnit         ADV Timer通道选择（TIM4、TIM5、TIM6）
 *
 * \retval  boolean_t  返回 1:  计数器开启
 * \retval  boolean_t  返回 0:  计数器关闭 
 */
boolean_t Adt_Start_Status(en_adt_unit_t enAdtUnit)
{
		boolean_t	adt_start_status;
    volatile M0P_ADT_TypeDef *pstcM0PAdt = (M0P_ADT_TypeDef *)((uint32_t)M0P_ADT0+0x400*enAdtUnit);
    ...
    adt_start_status	=	pstcM0PAdt->GCONR_f.START;

    return adt_start_status;
}
/**
 * \brief   
*          获取上溢中断标志
 *         自己添加的
 * \param   [in]  enAdtUnit     ADV Timer通道选择（TIM4、TIM5、TIM6）
 * \param   [in]  enAdtIrq      OVFF
 *
 */
boolean_t Adt_GetOvffIntFlag(en_adt_unit_t enAdtUnit)
{
